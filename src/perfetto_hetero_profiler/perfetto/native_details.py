...
def _chrome_detail_result(
    ...
    *,
    ...
    """Compatibility entry point for the Kineto provider implementation."""

    from .native_kineto import chrome_detail_result

    return chrome_detail_result(
        loaded,
        source,
...
def _nsys_detail_result(
    ...
    *,
    ...
    """Compatibility entry point for the Nsight SQLite provider."""

    from .native_nsys import nsys_detail_result

    return nsys_detail_result(
        loaded,
        source,
        native_clock_domain=native_clock_domain,
        native_timestamp_unit=native_timestamp_unit,
...
def _read_nsys_rows(
    ...
    bridge: _ClockBridge,
    native_epoch_base_ns: int,
) -> tuple[list[_NativeSlice], dict[str, TrackSpec], Counter[str], int]:
    """Compatibility entry point for existing Nsight helper tests."""

    from .native_nsys import _read_nsys_rows as read_nsys_rows
...
def _rbln_native_only_result(
    ...
    *,
    ...
    """Compatibility entry point for the RBLN Perfetto provider."""

    from .native_rbln import rbln_native_only_result

    return rbln_native_only_result(
        source,
        native_clock_domain=native_clock_domain,
        native_timestamp_unit=native_timestamp_unit,
    )
